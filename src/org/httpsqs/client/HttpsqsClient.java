  ...
  private String doGetProcess(String urlstr, String user, String pass) {
    ...
    try {
      ...
      if (user != null && pass != null) {
        conn.setRequestProperty("Authorization", "Basic " + new String(Base64.encodeBytes((user + ":" + pass).getBytes(charset))));  //需要BASIC验证
      }
...
//synctime
  ...
  public String flush(String queue_name, String user, String pass) {
    String result = null;
    try {
      String urlstr = "http://" + this.server + ":" + this.port + "/?name=" + URLEncoder.encode(queue_name, charset) + "&opt=flush";
      result = this.doGetProcess(urlstr, user, pass);
      return result;
    } catch (UnsupportedEncodingException ex) {
      return HTTPSQS_ERROR_PREFIX + ":" + ex.getMessage();
  ...
  public String view(String queue_name, long pos, String auth) {
    String result = null;
    try {
      StringBuilder urlstr = new StringBuilder("http://" + this.server + ":" + this.port + "/?charset=" + this.charset + "&name=" + URLEncoder.encode(queue_name, charset) + "&opt=view&pos=" + pos);
      if (auth != null) {
  ...
  public String get(String queue_name, String auth) {
    String result = null;
    try {
      StringBuilder urlstr = new StringBuilder("http://" + this.server + ":" + this.port + "/?charset=" + this.charset + "&name=" + URLEncoder.encode(queue_name, charset) + "&opt=get");
      if (auth != null) {
        urlstr.append("&auth=" + URLEncoder.encode(auth, charset));
      }
  ...
  public String put(String queue_name, String data, String auth) {
    ...
    URL url;
    try {
      urlstr = new StringBuilder("http://" + this.server + ":" + this.port + "/?name=" + URLEncoder.encode(queue_name, charset) + "&opt=put");
      if (auth != null) {
        urlstr.append("&auth=" + URLEncoder.encode(auth, charset));
      }
